    ...
    def forward(self, x):
        ...
        if self.fused_resample and self.up and w is not None:
            ...
            if self.up:
                ...
                if DEBUG: print(' fpad: {}'.format(f_pad))

                if YAMIN:
                    x = torch.nn.functional.interpolate(x,size=(x.shape[-2]*2,x.shape[-1]*2),mode='bilinear') # YAMIN ADDITIONS
                else:
            ...
            if self.down:
                ...
                if YAMIN:
                    ...
                    x = torch.nn.functional.conv2d(x, f.tile([self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad) # ORIGINAL CODE
                if DEBUG: print(x.shape)
            if w is not None:
...
class SongUNet(torch.nn.Module):
    def __init__(self,
        ...
        for level, mult in enumerate(channel_mult):
            if DEBUG: print('level: {}'.format(level))
            if DEBUG: print('mult:  {}'.format(mult))
    ...
    def forward(self, x, noise_labels, class_labels, augment_labels=None):
        ...
        if DEBUG: print("input encoder: {}".format(x.shape))
        for name, block in self.enc.items():
            if 'aux_down' in name:
                ...
                skips.append(x)
            if DEBUG: print("output encoder, name: {} block: {}".format(name,block))
        ...
        for name, block in self.dec.items():
            if 'aux_up' in name:
                if DEBUG: print('aux_up')
                aux = block(aux)
            elif 'aux_norm' in name:
                if DEBUG: print('aux_norm')
                tmp = block(x)
            elif 'aux_conv' in name:
                if DEBUG: print('aux_conv')
                tmp = block(silu(tmp))
                    ...
                    if YAMIN:
                        ...
                        if x.shape[-2] != skips[-1].shape[-2] or x.shape[-1] != skips[-1].shape[-1]:
                            if DEBUG: 
                                ...
                                print('   skips[-1].shape: {}'.format(skips[-1]))
